...
async def test_redis_trace_logging():
    ...
    # Create config
    config = RedisConfig(host="localhost", port=6379, enable_trace_logging=True)
        ...
        with patch("smap_shared.redis.client.ConnectionPool"):
            ...
            with patch("smap_shared.redis.client.logger") as mock_logger:
                ...
                assert trace_id in log_message, (
                    f"Trace ID not found in log: {log_message}"
                )
                assert "operation=GET" in log_message, (
                    f"Operation not found in log: {log_message}"
                )
                assert "key=test:key" in log_message, (
                    f"Key not found in log: {log_message}"
                )
    ...
    with patch("smap_shared.redis.client.aioredis.Redis"):
        with patch("smap_shared.redis.client.ConnectionPool"):
            ...
            with patch("smap_shared.redis.client.logger") as mock_logger:
                ...
                mock_logger.info.assert_called_once()
                log_message = mock_logger.info.call_args[0][0]
                assert "trace_id=" not in log_message, (
                    f"Unexpected trace_id in log: {log_message}"
                )
                assert "operation=SET" in log_message, (
                    f"Operation not found in log: {log_message}"
                )
