...
class TracedHTTPClient:
        ...
        **client_kwargs,
        ...
        if self.use_async:
            ...
            if not REQUESTS_AVAILABLE:
                raise ImportError(
                    "requests is required for sync HTTP client. Install with: pip install requests"
                )
    ...
    async def _create_async_client(self) -> "httpx.AsyncClient":
        ...
        client = httpx.AsyncClient(
            timeout=self.timeout, transport=transport, **self.client_kwargs
        )
        ...
        **kwargs,
        ...
        try:
            response = await self._client.get(
                url, headers=prepared_headers, params=params, **kwargs
            )
    ...
    def delete_sync(
        ...
        try:
            response = self._client.delete(url, headers=prepared_headers, **kwargs)
            return response.content, response.status_code
        except Exception as e:
            logger.error(f"HTTP DELETE request failed: {url} - {e}")
            raise
